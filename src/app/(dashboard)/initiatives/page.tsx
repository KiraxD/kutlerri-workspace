...
import { Compass, Plus, Target, ArrowRight, TrendingUp, Calendar, User } from 'lucide-react'
    ...
    <div className="flex flex-col bg-background">
      ...
      {initiatives.length > 0 && (
        <div className="flex gap-6 px-8 py-3 border-b border-border/60 bg-muted/20 text-sm">
          <span className="text-muted-foreground">Total: <strong className="text-foreground">{initiatives.length}</strong></span>
          ...
          <span className="text-muted-foreground">Blocked: <strong className="text-red-600">{grouped['Blocked']?.length ?? 0}</strong></span>
        </div>
      )}
      ...
      <div className="flex-1 p-8">
        {initiatives.length === 0 ? (
          <EmptyState
            ...
            action={<Button className="gap-2"><Plus className="w-4 h-4" /> Create Initiative</Button>}
            prereq={orgIds.length === 0 ? <Link href="/teams" className="text-sm text-primary hover:underline">Create a team first →</Link> : null}
          />
        ) : (
    ...
    <div className="flex items-center gap-4 p-4 rounded-xl border border-border bg-card hover:border-primary/30 hover:shadow-sm transition-all cursor-pointer group">
      ...
      <div className="flex-1 min-w-0">
        ...
        {initiative.description && (
          <p className="text-xs text-muted-foreground truncate mt-0.5">{initiative.description}</p>
        )}
      </div>
